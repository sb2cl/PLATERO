...
% clab: optional string with the colour label. Default value set to "".
...
arguments
    ...
    y 
    g 
    xdiscrete (1,1)double=0
    ydiscrete (1,1)double=0
    disp (1,1)double=0.01*min(abs(x))
    xlab {char,string}=""
    ylab {char,string}=""
    clab {char,string}=""
    titlab {char,string}=""
...
for i = 1:length(ug)
    ...
    scatter(xnoisy,ynoisy,20,'Marker',symbs{i},'MarkerFaceColor',...
        colscale(i,:),'MarkerFaceAlpha',0.6,'MarkerEdgeColor',edcol{i},...
        'Linewidth',1.2,'HandleVisibility','on','DisplayName',strcat(string(uug(i)))),
    hold on
end
grid on
